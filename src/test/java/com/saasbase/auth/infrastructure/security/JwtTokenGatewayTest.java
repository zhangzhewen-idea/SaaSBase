...
class JwtTokenGatewayTest {
    ...
    void signs_and_parses_access_token() {
        ...
        UserPrincipal principal = new UserPrincipal(1001L, 2001L, "alice", Set.of("tenant:user:read"), 4L, true);
    ...
    @Test
    void preserves_custom_claims_round_trip() {
        JwtTokenGateway gateway = new JwtTokenGateway("01234567890123456789012345678901", Duration.ofMinutes(15));
        UserPrincipal principal = new UserPrincipal(1001L, 2001L, "alice", Set.of("tenant:user:read"), 9L, false);

        UserPrincipal parsed = gateway.parseAccessToken(gateway.issueAccessToken(principal));

        assertThat(parsed.sessionVersion()).isEqualTo(9L);
