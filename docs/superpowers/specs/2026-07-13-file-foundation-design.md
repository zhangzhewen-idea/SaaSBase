...
- 文件 REST 接口已接入 `/api/v1/admin/files`。
- 上传、详情、列表、内容读取和删除已接入应用层。
- `FileView` 作为对外返回模型，隐藏了 `objectKey` 和存储实现细节。
- 列表查询支持默认分页参数，空参数不会导致 400。
- 本地存储适配器和 MyBatis 元数据持久化已完成。
- 文件模块单测和全量测试已通过。
...
配置前缀仍为 `saasbase.file`，与 `FileStorageProperties` 保持一致。
